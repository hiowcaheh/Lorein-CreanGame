...
| `web/src/ekrany/Lochy.tsx` — przejsty loch | Kafla przejsciowego lochu nie da sie kliknac; ekran lochu sie nie otwiera | Oryginal na PIERWSZEJ planszy otwiera go normalnie i tylko wygasza przycisk walki (`CNT_MAINQUEST_GREY`), a na DRUGIEJ klikniecie dowolnego kafla i tak pokazuje loch BIEZACY, a nie klikniety (`if (countDone == 9) ShowMainQuestScreen(9, ...)`, i tak dalej az do 12) — wiec przejsciowego lochu nie da sie tam obejrzec wcale. Wlasciciel gry uznal otwieranie przejsciowego lochu za blad: nie ma tam czego robic, a ekran i tak zapowiada „Kolejny przeciwnik", ktorego juz nie ma. Kafel jest wiec zablokowany na OBU planszach. |
