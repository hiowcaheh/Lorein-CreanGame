...
| `web/src/ekrany/karczma/OknoWyboru.tsx` — premie | Przy nagrodzie podbitej premia stoi ikonka „+", a klikniecie doswiadczenia rozpisuje, ile dokłada klaser i rzadkie zadanie | Oryginal nie oznacza takiej nagrody niczym — podwiesza tylko podpowiedz (`EnablePopup(LBL_QO_REWARDEXP, ...)`), a na dotyku nie ma czego najezdzac. Wlasciciel gry poprosil o widoczny znak; ikonka to `btnClassPlus` z oryginalu. Sama tresc rozpisania jest jeden do jednego: „w tym" (`TXT_EXPBONUS_PREFIX`) i „Premia kolekcjonera" (`TXT_COLLECTION + 1`). Liczba doswiadczenia jest juz z premiami — tak samo, jak podaje ja `req.php`. |
