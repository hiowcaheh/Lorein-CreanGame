...
| `web/src/ekrany/Lochy.tsx` — licznik przerwy | Ile zostalo do konca przerwy, stoi NA przycisku („OK 59:38 (1~P)"), a nie osobnym napisem obok | Oryginal ma tam `LBL_MAINQUEST_MUSHHINT` po lewej od przycisku, ale to miejsce zajmuje u nas „Wroc" — oryginal takiego przycisku nie ma, wychodzi sie krzyzykiem. Pelne zdanie z `TXT_MQ_MUSHHINT` zostalo jako podpowiedz przycisku. |
...
| `backend/src/api/lochy.ts` — odlamki lustra i klucz do wychodka | Z rodzaju 11 wypada tylko klucz do lochu; odlamka Magicznego Lustra ani klucza do wychodka nie ma | Obu ekranow jeszcze nie ma, wiec nie ma czego otwierac. Same LOSOWANIA (`rand(1, 2)` trzy razy) zostaja w kodzie i sa zuzywane — bez nich caly dalszy ciag generatora rozjechalby sie z oryginalem. |
