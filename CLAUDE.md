...
| `web/src/gra/liczby.ts` — `skrocona()` i `klasaWielkosci()` | Kwoty od miliona w gore stoja skrocone („18kk", „1,5kkk") i w kolorze: od miliona zielone (`--liczba-duza`), od dziesieciu milionow czerwone (`--liczba-ogromna`). Dokladna kwota wychodzi po klikniecu | Oryginal nie skraca nic i nie koloruje nic. Przy kilkunastu milionach srebra pelny zapis nie miescil sie w pasku zasobow i wychodzil poza ramke panelu, a w oknie po walce wchodzil na ikone zdobyczy. Wlasciciel gry poprosil o skrot w zapisie z gier przegladarkowych i o kolory po progach. Skrot NIGDY nie zaokragla w gore — kwota nie moze wygladac na wieksza, niz jest. |
...
| `web/src/gra/ekranPostaci.ts` — `KLASER` | Ikonka klasera stoi przy przycisku „Klaser", a nie w prawym gornym rogu | Oryginal stawia `IMG_CHAR_ALBUM` w (280 + 500 + 350, 100 + 20), z dala od czegokolwiek, co klasera dotyczy. Wlasciciel gry poprosil o przeniesienie jej do przycisku. Nowe polozenie jest WYLICZONE z przycisku (`left: 550`, wysokosc 45), a nie zmierzone na oko. |
