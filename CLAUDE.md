...
| `web/src/gra/liczby.ts` — `klasaWielkosci()` oraz `Zasoby` w `App.tsx` | Kwoty stoja W CALOSCI, bez skracania, za to w kolorze: od miliona zielone (`--liczba-duza`), od dziesieciu milionow czerwone (`--liczba-ogromna`). Na pasku zasobow SREBRO pokazuje sie tylko wtedy, gdy nie ma ani jednego zlota; dokladne kwoty wychodza po klikniecu | Oryginal nie koloruje nic i pokazuje zawsze zloto RAZEM ze srebrem. Przy kilkunastu milionach cala kwota nie miescila sie w pasku i wychodzila poza ramke panelu. Skracanie („18kk") bylo probowane i wlasciciel gry je ODRZUCIL — skrot gubi koncowke, a gracz chce widziec dokladna sume. Miejsce robi wiec schowanie srebra: ponizej stu srebra nic wartego tego miejsca nie mowi. |
...
| `web/src/gra/ekranPostaci.ts` — `KLASER` | Ikonka klasera stoi przy przycisku „Klaser", a nie w prawym gornym rogu | Oryginal stawia `IMG_CHAR_ALBUM` w (280 + 500 + 350, 100 + 20), z dala od czegokolwiek, co klasera dotyczy. Wlasciciel gry poprosil o przeniesienie jej do przycisku. Polozenie jest WYLICZONE z przycisku (POMIAR: 550, 615, 174x50), a nie zmierzone na oko: ikonka staje 10 px ZA nim (734) i o 1 px nizej, srodkiem w jego srodku. Po prawej, bo z lewej strony przycisku zostaje juz tylko waski pasek do krawedzi ekranu.
