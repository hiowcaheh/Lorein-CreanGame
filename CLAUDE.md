...
| `backend/src/api/lochy.ts` i `game/karczma.ts` — `zlotoZWyprawy()` | Premia kolekcjonera podbija ZLOTO — i w lochu, i na wyprawie | Oryginal liczy zloto CALKIEM innym wzorem niz doswiadczenie i klasera tam nie ma: `$gold = quest_gold * ($gbonus + $towerbonus)`, gdzie `$gbonus = 1 + (treasure + dung) / 50` (komnata skarbow w gildii i przejsciowe lochy), a `$albumbonus` wchodzi WYLACZNIE do doswiadczenia (`$exp += quest_exp * ($ebonus + $albumbonus + $rqbonus)`). W lochu oryginal nie doklada zadnej premii — bierze `$OP->getExp()` i `$OP->getGold()` surowo. Wlasciciel gry zdecydowal, ze klaser ma liczyc sie w obu miejscach i dotyczyc obu nagrod. Premie zlota z oryginalu (skarbiec, lochy gildii, wieza) SA w kodzie i sa zerami — czekaja na gildie i wieze. Rzadkiego zadania zloto nie dotyczy, tak jak w oryginale. Kolejnosc dzialan i zaokraglenia bez zmian: mnozenie, potem setki powyzej dziesieciu tysiecy i sufit miliarda. |
